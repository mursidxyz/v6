...
var handler = async (m, { conn, command, args }) => {
  ...
  await m.reply('📮Sabar lagi Ss web nya😁');
  var url = args[0].startsWith('http') ? args[0] : 'https://' + args[0]
  try {
    var img = await fetch(`https://xzn.wtf/api/ssweb?type=dekstop&url=${url}&apikey=Akiraa`);
    if (!img) {
      await m.reply('Gagal saat percobaan pertama. Memulai percobaan kedua...');
      img = await fetch(`https://xzn.wtf/api/ssweb?type=dekstop&url=${url}&apikey=Akiraa`);
